...
// Update your getImageUrl function to reduce logging
...
const RetailerInventoryManagement = () => {
  ...
  const sidebarItems = [
    { id: "dashboard", name: "Dashboard", icon: LayoutDashboard, path: "/retailer/dashboard" },
    { id: "inventory", name: "Inventory", icon: Boxes, path: "/retailer/inventory" },
    { id: "catalog", name: "Catalog", icon: FileText, path: "/retailer/catalog" },
    { id: "orders", name: "Orders", icon: ShoppingCart, path: "/retailer/orders" },
    { id: "ai", name: "AI Module", icon: BrainCircuit, path: "/retailer/ai" }
  ];
  ...
  const unitOptions = ['kg', 'pieces', 'boxes', 'packets'];
  ...
  const fetchInventory = async () => {
    try {
      setLoading(true);
      // Using retailer-specific user ID from localStorage
      ...
      if (response.data.success) {
        ...
        console.log('📦 Retailer Inventory data received:');
  ...
  const fetchLiveSensors = async () => {
    try {
      // Simulate fetching live sensor data
      const simulatedData = {
        temperature: (Math.random() * 8 - 2).toFixed(1),
        humidity: (Math.random() * 30 + 70).toFixed(1),
        ammonia: (Math.random() * 2 + 0.5).toFixed(1)
      };
      ...
      alert('IoT Sensors auto-filled with simulated data!');
    } catch (error) {
      console.error("Error fetching live sensor data:", error);
      alert("Failed to load live sensor values");
    }
  };
...
const addInventoryItem = async () => {
  try {
    ...
    if (quantityNum > 100000 || priceNum > 100000) {
      alert('Values cannot exceed 100,000. Please enter reasonable values.');
      return;
    }
    ...
    const batchId = `DIST-${seafoodInitials}-${timestamp}-${randomNum}`;
  ...
  try {
    ...
    if (editingItem.quantity && Number(editingItem.quantity) > 100000) {
      alert('Quantity cannot exceed 100,000. Please enter a reasonable value.');
      return;
    }
    
    if (editingItem.price && Number(editingItem.price) > 100000) {
      alert('Price cannot exceed 100,000. Please enter a reasonable value.');
      return;
    }
        ...
        <header className="bg-white border-b border-gray-200 shadow-sm">
          <div className="px-6 py-4">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-bold text-gray-800">Retailer Inventory Management</h2>
                <p className="text-sm text-gray-600 mt-1">Manage retail seafood inventory and monitor quality metrics</p>
              </div>
            </div>
          </div>
                              ...
                              {item.image ? (
                                <div className="w-16 h-16 rounded-lg overflow-hidden border border-gray-200 relative group">
                                  <img 
                                    ...
                                    onLoad={() => console.log(`✅ Retailer image loaded: ${getImageUrl(item.image)}`)}
                                    onError={(e) => {
                                      console.error(`❌ Retailer image failed: ${getImageUrl(item.image)}`);
                                      console.error('Original path:', item.image);
                                      e.target.onerror = null;
                                      e.target.src = `data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="%23ff6b6b"><title>Path: ${item.image}</title><path d="M21 5v14H5V5h16m0-2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-3.5 12h-2v2h2v-2zm0-4h-2v2h2v-2zm-6 4h-2v2h2v-2zm0-4h-2v2h2v-2z"/></svg>`;
                                    }}
                                  />
        ...
        {showAddModal && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white shadow-2xl rounded-2xl max-w-5xl w-full p-6 border border-gray-200">
              <h3 className="text-lg font-bold text-[#012A4A] mb-6">Add New Retail Stock</h3>
...
/>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-[#0A3D62] mb-2">Retail Price per Unit ($)</label>
                    <input
  type="number"
  step="0.01"
  value={newItem.price}
...
/>
                ...
                {/* Right Column - IoT Sensors */}
                <div className="space-y-4">
                  <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                    <h4 className="font-medium text-[#0A3D62] mb-4 flex items-center gap-2">
                      <Thermometer className="w-4 h-4" />
                      Storage Sensor Readings
                    </h4>
                    
                    <div className="space-y-4">
                      <div>
                        <label className="block text-sm font-medium text-[#0A3D62] mb-2">Temperature (°C)</label>
                        <input
                          type="number"
                          step="0.1"
                      ...
                      <div>
                        <label className="block text-sm font-medium text-[#0A3D62] mb-2">Humidity (%)</label>
                        <input
                          type="number"
                          step="0.1"
                      ...
                      <div>
                        <label className="block text-sm font-medium text-[#0A3D62] mb-2">Ammonia (ppm)</label>
                        <input
                          type="number"
                          step="0.1"
                          value={newItem.ammonia}
                          onChange={(e) => setNewItem({ ...newItem, ammonia: e.target.value })}
                          className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1B98E0] focus:border-[#1B98E0] bg-white text-[#012A4A]"
                        />
                      </div>
                    </div>
...
export default RetailerInventoryManagement;