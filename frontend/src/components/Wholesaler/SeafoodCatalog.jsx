...
class SocketManager {
  ...
  initialize(userId, role = 'wholesaler') {
...
const useSocket = () => {
  ...
  useEffect(() => {
    ...
    if (userData.id && !socketManager.isConnected()) {
      socketManager.initialize(userData.id, userData.role || 'wholesaler');
    }
...
const WholesalerSeafoodCatalog = () => {
  const navigate = useNavigate();
...
export default function WholesalerSeafoodCatalogWrapper() {
  return (
    <CartProvider>
      <WholesalerSeafoodCatalog />
    </CartProvider>
  );
}