...
const loadNotificationsFromStorage = (userId) => {
  try {
    if (!userId) return [];
    const storageKey = `supplier_notifications_${userId}`;
...
const SupplierDashboard = () => {
  ...
  const [notifications, setNotifications] = useState(() => {
    ...
    if (storedUser?.id) {
      try {
        const storageKey = `supplier_notifications_${storedUser.id}`;
        const saved = localStorage.getItem(storageKey);
        if (saved) {
...
const fetchSupplierOrders = async () => {
  try {
    const storedUser = JSON.parse(localStorage.getItem('userData') || sessionStorage.getItem('userData'));
    if (storedUser?.id) {
      console.log(`🔄 Fetching orders for supplier: ${storedUser.id}`);
      console.log(`🔄 Using URL: ${API_BASE_URL}/orders/supplier/${storedUser.id}`);
      
      const response = await axios.get(`${API_BASE_URL}/orders/supplier/${storedUser.id}`, {
        timeout: 5000,
        params: {
          status: 'pending',
          limit: 10
...
const loadExistingOrders = async () => {
  ...
  useEffect(() => {
    console.log('🔌 Setting up socket connection for supplier...');
    ...
    console.log(`👤 Supplier ID: ${storedUser.id}`);
    ...
    socketInstance.on('connect', () => {
      ...
      console.log(`🏭 Emitted join-supplier for supplier-${storedUser.id}`);
    });
...
// In the socket event handler for 'new_order_for_supplier':
socketInstance.on('new_order_for_supplier', (data) => {
  console.log('🎯🎯🎯 [SOCKET EVENT] new_order_for_supplier RECEIVED:', data);
  ...
  setNotifications(prev => {
    ...
    try {
      localStorage.setItem(`supplier_notifications_${storedUser.id}`, JSON.stringify(newState));
    } catch (error) {
...
socketInstance.on('newOrder', (data) => {
  ...
  const handleMarkAsRead = (notificationId) => {
    ...
    setNotifications(prev => {
      ...
      // Save to localStorage
      if (storedUser?.id) {
        localStorage.setItem(`supplier_notifications_${storedUser.id}`, JSON.stringify(updated));
      }
