...
const InventoryManagement = () => {
  ...
  const sidebarItems = [
    { id: "dashboard", name: "Dashboard", icon: LayoutDashboard, path: "/supplier/dashboard" },
    { id: "inventory", name: "Inventory", icon: Boxes, path: "/supplier/inventory" },
    { id: "orders", name: "Order", icon: ShoppingCart, path: "/supplier/order" },
    { id: "ai", name: "AI Module", icon: BrainCircuit, path: "/supplier/ai" }
  ];
  ...
  const updateInventoryItem = async () => {
    ...
    try {
      ...
      // Add user_id (NOT userId) - this is the key fix!
      const userId = localStorage.getItem('user_id');
      if (userId) {
        formData.append('user_id', userId);  // Use underscore!
      ...
      if (editingItem.imageFile) {
        ...
        // Base64 image from editing (when user edits existing image)
      ...
      // Add userType for backend (will be filtered out)
      formData.append('userType', 'retailer');
                              ...
                              {item.image ? (
                                <div className="w-16 h-16 rounded-lg overflow-hidden border border-gray-200 relative group">
                                  <img 
                                    ...
                                    onLoad={() => console.log(`✅ Supplier image loaded: ${getImageUrl(item.image)}`)}
                                    onError={(e) => {
                                      console.error(`❌ Supplier image failed: ${getImageUrl(item.image)}`);
                                      console.error('Original path:', item.image);
                                      e.target.onerror = null;
                                      // Show broken image icon with path info
