...
const DistributorInventoryManagement = () => {
  ...
  const fetchInventory = async () => {
    try {
      setLoading(true);
      // Using distributor-specific user ID from localStorage
      ...
      if (response.data.success) {
        ...
        console.log('📦 Distributor Inventory data received:');
  ...
  const generateLiveSensors = () => {
    // Generate realistic sensor data for distributor storage
    const simulatedData = {
    ...
    alert('✅ Storage sensor data auto-generated!\n' +
          `🌡️ Temperature: ${simulatedData.temperature}°C\n` +
          `💧 Humidity: ${simulatedData.humidity}%\n` +
          `⚠️ Ammonia: ${simulatedData.ammonia}ppm`);
  };

  // Auto-generate sensors when modal opens
  useEffect(() => {
    if (showAddModal) {
      generateLiveSensors();
    }
  ...
  const addInventoryItem = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        fetchInventory();
        alert(`Distribution stock added successfully!\nBatch ID: ${response.data.batchId}`);
      }
    } catch (error) {
        ...
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-blue-700">Distributor Portal</p>
            </div>
          </div>
        </div>
        ...
        <header className="bg-white border-b border-gray-200 shadow-sm">
          <div className="px-6 py-4">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-bold text-gray-800">Distributor Inventory Management</h2>
                <p className="text-sm text-gray-600 mt-1">Manage distribution seafood inventory and monitor quality metrics</p>
              </div>
            </div>
          </div>
        ...
        {showAddModal && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white shadow-2xl rounded-2xl max-w-5xl w-full p-6 border border-gray-200">
              <h3 className="text-lg font-bold text-[#012A4A] mb-6">Add New Distribution Stock</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-[#0A3D62] mb-2">Distribution Price per Unit ($)</label>
                    <input
                      type="number"
                      step="0.01"
                      value={newItem.price}
                ...
                <div className="space-y-4">
                  <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                    <h4 className="font-medium text-[#0A3D62] mb-4 flex items-center gap-2">
                      ...
                      Storage Sensor Readings (Auto-generated)
                    </h4>
...
export default DistributorInventoryManagement;